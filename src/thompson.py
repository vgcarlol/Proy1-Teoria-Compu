class State:
    ...
    def add_transition(self, symbol, state):
        ...
        print(f"  Añadida transición: {symbol} -> Estado({id(state)})")  # Depuración para transiciones normales

    def add_epsilon_transition(self, state):
        self.epsilon_transitions.add(state)
        print(f"  Añadida transición epsilon -> Estado({id(state)})")  # Depuración para transiciones epsilon
...
def construct_afn(postfix_expression):
    ...
    for char in postfix_expression:
        if char.isalnum() or char == 'ε':  # Símbolo básico o epsilon
            ...
            # Conectar el primer AFN con el segundo
            afn1.accept_state.add_epsilon_transition(afn2.start_state)
            afn1.accept_state.is_accept = False  # Marcar el estado intermedio como no aceptante
            ...
            # Epsilon transiciones para la estrella de Kleene
            start_state.add_epsilon_transition(afn1.start_state)
            start_state.add_epsilon_transition(accept_state)
            afn1.accept_state.add_epsilon_transition(afn1.start_state)
            afn1.accept_state.add_epsilon_transition(accept_state)
            afn1.accept_state.is_accept = False  # Marcar el estado anterior como no aceptante
    ...
    # Debug: Listar transiciones finales del AFN
    visited_states = set()
